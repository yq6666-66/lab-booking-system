...
- **组合规则集成测试 T63–T67**：审批×HELD（PENDING 不占容 + 批准重校 + 递补绕审批落 HELD + confirm）、优先级×可执行 FIFO（priority DESC 暂跳高优先级冲突者、strict 对照整体停止）、抢占×信用（借 WEEKLY 回补标记做确定性余额控制，3→4 补偿未触顶 + 再预约）、维护×资源声明（409/既有保留/恢复后可声明）、改期×周配额（计数守恒 + 改期不放水）。集成断言 63 → 68。
