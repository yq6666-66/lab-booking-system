...
slide("四、数据模型（schema v2→v5）", img="fig4-2-er.png", note="10 张表；v2 签到/取消原因/通知表；v3 容量制；v4 提醒防重 + CHECK 重建；v5 资源表 assets（实验室内名唯一、三态）。全部幂等迁移，旧库实测。")
...
slide("八、管理控制台与运营能力", img="fig5-1-admin-overview.png", note="独立控制台十一页签：场次开始前调整、用户管理（近 7 天爽约列与「受限中」标记）、资源管理（设备清单三态维护）、通知发布、运行日志筛选；统计页含利用率表与 SVG 图，零依赖。运营：开场提醒（防重）、备份轮转、30 天归档、--demo-days 演示数据。")
slide("九、验证体系：五层自动化", bullets=[
    ("单元测试 24 个（纯函数/不变量/业务直调/限流纯函数）", 0),
    ("集成实验 44 项断言组（独立 Python 客户端驱动真实进程）", 0),
