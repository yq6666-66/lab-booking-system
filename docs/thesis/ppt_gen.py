...
slide("三、需求与业务规则（BR1–BR21）", bullets=[
    ...
    ("候补策略三档：可执行 FIFO（BR19 默认）/ strict / weighted 老化加权（1000·优先级+200·信用+60·log₂ 等待）；HELD 限时保留（BR20）", 0),
...
slide("五、核心机制 1：并发一致性", bullets=[
    ...
    ("事务内校验容量不变量：占容（CONFIRMED+HELD）≤ capacity", 0),
    ("唯一索引兜底（v1）→ 事务协议保证（v3）的演进", 0),
    ("实证：720 次并发争抢，每轮成功数与落库占用数恒为 1，实验条件下未出现超卖", 0),
])
...
slide("八、管理控制台与运营能力", img="fig5-1-admin-overview.png", note="独立控制台十二页签：场次调整、待审批专页（批量审批）、用户管理、资源管理（三态维护）、通知发布、运行日志；统计页含利用率表与 SVG 图；全员记录表内置强制取消/代签退。用户端预约卡可勾选资源声明，同时段配额事务内校验（BR12），取消自动释放。运营：提醒、备份轮转、归档、聚合概览/冲突检测/信用概览（v1.17.0）、--demo-days。")
slide("九、验证体系：五层自动化", bullets=[
    ("单元测试 25 个（纯函数/不变量/业务直调/限流纯函数/outbox 派发）", 0),
    ("集成实验 79 项断言组（T01–T78 及 CLI；组合验证 T63–T67、T68–T69、策略治理 T70–T73、差分属性 T74、管理操作 T75–T78）", 0),
    ("故障注入三类各 10 轮共 30 次：提交前 86 / 提交后 87 / 扫描中途 88，全部正确", 0),
    ("模糊 720 次恶意输入零崩溃；浸泡 30,892 请求零错误", 0),
    ("CI 三通道门禁：构建与测试 / 静态分析 / AddressSanitizer 内存安全（llvm-mingw）", 0),
    ("计时旁路实验：登录有效/无效账号响应时间比 1.07，显著降低账号枚举可观测差异", 0),
])
slide("十、并发争抢实验", bullets=[
    ("1/5/10/20 并发 × 20 轮 = 720 次争抢", 0),
    ("每轮成功预约数 = 数据库占用数 = 1（实验样本内未出现超卖）", 0),
    ("其余请求全部收到明确 409，无忙碌失败", 0),
    ("该实验条件下，事务协议足以替代唯一索引兜底；三策略矩阵：strict 队首冲突空置，executable/weighted 满额递补且 weighted 平均等待减半（7.2ks→3.6ks）", 0),
])
slide("十一、性能优化实验", bullets=[
    ("优化点：每线程连接复用 + 预编译语句 LRU 缓存", 0),
    ("读路径：20 并发吞吐 304 → 1948 rps（+541%）", 0),
    ("读路径 p50：63.9ms → 8.2ms（-87%）", 0),
    ("写路径受写锁约束：+2%~+63%，符合预期", 0),
    ("方法：同机同参前后对比，数据与脚本全部归档", 0),
])
slide("十二、覆盖率与稳健性", bullets=[
    ("gcov 行覆盖率：全模块 81.5% ~ 98.1%", 0),
    ("模糊实验 720 次恶意输入：零崩溃、零 5xx", 0),
    ("浸泡 30,892 请求：工作集 +2.8MB、句柄 +38，浸泡窗口内未观察到持续增长", 0),
    ("七个真实缺陷由测试先行暴露并回归固化", 0),
])
slide("十三、总结与展望", bullets=[
    ("结论：C + 轻组件 + 事务纪律 + 自动化实验 = 可论证的可靠性", 0),
    ("一体化深化：资源声明配额、审批流、候补三档策略（含老化加权）、资格授权、信用账户、约束感知建议（含转正概率）、公平性审计（Jain 指数）、每日候补上限", 0),
