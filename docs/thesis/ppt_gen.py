...
slide("三、需求与业务规则（BR1–BR11）", bullets=[
    ...
    ("管理端：独立控制台——场次调整、用户管理、通知发布、运行日志", 0),
])
slide("四、数据模型（schema v2→v4）", img="fig4-2-er.png", note="9 张表；v2 签到/取消原因/通知表；v3 容量制、退役唯一索引；v4 提醒防重加列 + CHECK 约束重建（建新表—拷贝—改名）。全部幂等迁移，旧库实测升级。")
...
slide("八、管理控制台与运营能力", img="fig5-1-admin-overview.png", note="独立控制台十页签：场次开始前调整、用户管理（近 7 天爽约列与「受限中」标记）、通知发布、运行日志筛选；统计图为纯 DOM 生成的 SVG，零依赖。运营：场次开始提醒（reminded_at 防重）、自动备份轮转、30 天历史归档、--demo-days 演示数据。")
slide("九、验证体系：五层自动化", bullets=[
    ("单元测试 24 个（纯函数/不变量/业务直调/限流纯函数）", 0),
    ("集成实验 42 项断言组（独立 Python 客户端驱动真实进程）", 0),
...
slide("十三、总结与展望", bullets=[
    ...
    ("业务闭环延伸至运营：提醒、备份轮转、归档、爽约信用、重叠检测", 0),
