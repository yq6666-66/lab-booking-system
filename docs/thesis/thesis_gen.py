...
para("系统以单写者事务（BEGIN IMMEDIATE）配合容量不变量保证并发正确性；以持久化请求回执实现同编号同参数重放幂等、同编号异参数冲突拒绝；"
     ...
     "运行日志与资源清单）、限时签到、场次开始提醒、自动备份轮转、爽约信用约束与时间重叠检测等运营能力，并以全局安全响应头（CSP 等）收紧浏览器攻击面。")
para("系统通过五层自动化验证：24 个单元测试、42 项集成断言组（含 720 次并发争抢零超卖、30 次进程中断恢复全部正确）、契约与灰盒/文档测试、"
     "模糊稳健性与浸泡稳定性实验，以及持续集成三通道门禁（构建与测试、静态分析、AddressSanitizer 内存安全）。计时旁路实验表明登录路径"
     "对有效与无效账号的响应时间比仅为 1.07，无账号枚举侧信道。连接复用与语句缓存优化使读路径吞吐提升最高 600%，p50 延迟下降 86%。")
...
li("（3）设计并实现了面向真实运营的管理能力：独立管理员控制台（实验室与场次维护、场次开始前调整、用户停用/重置密码、全员通知发布、运行日志查看）、实验室资源（设备）清单与资源利用率统计、"
   "场次开始提醒、自动备份轮转、爽约信用约束（近 7 天两次爽约暂停预约）与跨场次时间重叠检测；")
li("（4）构建了五层自动化验证体系：单元测试 24 个、集成断言组 42 项（含 720 次并发争抢与 30 次故障注入恢复）、契约与灰盒/文档测试、模糊与浸泡实验，"
   "并以持续集成三通道门禁（构建与测试、静态分析、AddressSanitizer 内存安全）保障每一次提交；")
li("（5）完成性能与安全实验：每线程连接复用与语句缓存使读路径吞吐最高提升 600%；计时旁路实验证明登录路径不存在账号枚举侧信道。")
h2("1.4 论文组织结构")
...
para("系统包含两类角色。普通用户：查询开放场次、预约与取消、加入/退出候补、在签到窗口内签到、接收站内通知（补位、爽约、管理员公告、"
     "场次提醒）并批量已读、自助修改密码、管理本人在线会话、按状态筛选与翻页查看本人记录；管理员：维护实验室及其资源（设备）清单、按日期区间与容量发布场次、"
     "对已发布场次在开始前调整时间与容量、查看全体记录（按动作与用户筛选）与操作日志、管理用户（停用/启用、重置密码）、向全员或指定用户"
     "发布公告、查看通知送达与已读情况、查看逐日统计（图表与 CSV 导出）与运行指标、查看服务运行日志。管理员账号从登录页的管理员入口进入，"
     "认证通过后自动跳转独立管理控制台。账号由管理员初始化预置，本系统不开放自助注册。")
h2("3.2 功能需求")
para("核心功能需求归纳为：场次查询（按实验室与日期，返回容量与实时占用）；预约（容量内即时生效）；候补（满员排队，按入队顺序排队）；"
     ...
     "场次开始提醒（开始前可配置时段内向有效预约者推送站内提醒）；站内通知（补位、爽约、公告、提醒四类，分页加载）；账号自助管理与"
     "管理员用户管理（停用立即下线、重置密码一次性展示）；记录分页与状态筛选；逐日统计图表与 CSV 导出；服务运行日志查看与级别筛选；实验室资源清单维护（名称/规格/数量/可用状态）与按实验室的资源利用率统计导出。")
h2("3.3 业务规则")
...
h2("4.2 数据库设计")
para("数据模型共 10 张表，如图 4-2 所示。reservations（预约）与 waitlist（候补）是核心业务表；request_receipts（请求回执）支撑幂等去重；"
     "operation_events（操作事件）提供审计；notifications（通知）承载补位、爽约、公告与提醒四类站内消息；assets（r13 引入）承载各实验室的"
     "资源（设备）清单，以（实验室,资源名）唯一约束与三态状态（可用/维修中/停用）支撑资源管理。schema 采用版本化幂等迁移："
     "v2 追加签到与取消原因列及通知表；v3 为场次表追加 capacity 列并退役单占用唯一索引；v4 为场次表追加提醒防重列 reminded_at，并以"
     "“建新表—拷贝—改名”方式重建 CHECK 约束变化的通知表；v5 新增资源表 assets，全部迁移只追加、可重入，并以构造旧版本数据库的方式实测升级路径。")
img("fig4-2-er.png", cap_text="图 4-2 数据库 ER 图（schema v5）")
h2("4.3 状态转换设计")
...
para("api() 处理器依次执行 Host 白名单、方法检查、健康探测、Origin 校验、请求体上限与 JSON 合法性检查（含重复键拒绝）、数据库连接获取、"
     ...
     "同样返回 429；已登录用户的全部写操作经过每用户令牌桶（默认突发 30、每秒补充 1）限速。路由分发以统一分派器组织用户端与管理端共 20 余个"
     "端点，管理端点集中做角色断言，静态资源与 API 共享全局安全响应头。")
h2("5.3 事务与去重核心")
...
para("签到接口校验本人、CONFIRMED 状态与窗口期，以服务端时间落库，同编号重放与重复签到均返回首次签到时间。扫描线程以可配置间隔触发，"
     ...
     "补位预约的签到起点取其补位创建时刻，与场次的自然开始时间解耦。提醒任务复用同一线程循环：命中提醒窗口的场次向全部有效预约者写入 "
     "REMIND 通知并以 reminded_at 防重，重复扫描不会产生重复通知。")
h2("5.6 管理控制台与运营功能实现")
para("管理控制台为独立页面（admin.html + admin.js），以十一个页签组织实验室管理、场次发布、场次管理（开始前调整时间与容量）、全员记录"
     "（动作与用户双维筛选）、用户管理（前缀搜索、分页、停用/启用、重置密码一次性展示、近 7 天爽约计数与受限标记）、统计图表、运行指标、"
     "通知发布（全员或指定用户）、通知历史与运行日志（尾部读取、级别与行数筛选）。控制台概览如图 5-1 所示，用户管理如图 5-2 所示，"
     "统计图表如图 5-3 所示。资源管理页签维护实验室设备清单（名称/规格/数量/三态状态）；统计页的趋势图由前端以纯 DOM API 生成 SVG 双系列柱状图，不引入任何图表库。统计页还提供按实验室的资源利用率表与 CSV 导出。运营侧实现了自动备份线程"
...
h1("6 系统测试")
h2("6.1 测试策略与环境")
para("测试体系分五层：（1）Unity 单元测试 24 个，不经 HTTP 直接链接业务层与数据层；（2）集成实验 44 项断言组，以独立 Python 客户端"
...
h2("6.3 功能、安全与运营回归")
para("集成实验共 44 项断言组（T01–T42 及两组 CLI 检查），覆盖登录授权、占用唯一、候补 FIFO 与重入队尾、重放幂等、越权与 CSRF/Origin、"
     "畸形与超长输入、过期场次、外部写锁退避、禁用候补跳过、替代时段对账、统计对账、会话清理、签到窗口、分页参数、CSV 导出、改密会话失效、"
     "混合负载连接复用，以及运营扩展的角色登录、场次调整、通知发布、用户管理、场次提醒、自动备份、爽约信用、时间重叠、历史归档、日志查看与资源管理、利用率统计。"
     "最近一轮全部通过。典型新增加断言组如下：")
...
para("计时旁路实验：以脚本分别测量有效账号与不存在账号的登录响应时间各若干轮，二者中位数的比值稳定在 1.07（加固构建下 1.03），"
     "证明哑哈希验证成功抹平了 Argon2id 仅对有效账号执行所引入的时间差，登录路径不存在可用于账号枚举的计时侧信道。")
...
li("（1）SQL 绑定格式串错位（三例）：sessions_list 将字符串参数按 'is' 中的 i（8 字节）绑定，/api/slots 查询漏写占位符，演示数据生成器"
     "以四个绑定参数对应三个占位符——三种错位分别导致进程段错误、查不到任何场次与插入静默失败，均以最小化二分或逐项诊断输出定位，"
     "并在完整性检查中补充逐项原因打印；")
...
para("以真实 Chromium 浏览器完成多轮端到端验收：走通登录（含管理员入口）、查询、预约、候补、取消补位（两段式确认）、签到（含倒计时与"
     "状态机）、通知已读与分页、记录状态筛选、改密与会话下线、分页、统计图表与 CSV 下载、容量显示与替代时段改约、指标面板，以及管理控制台"
...
para("本课题面向高校开放实验室管理场景，完整经历了需求分析、架构设计、编码实现、自动化验证与性能优化五个阶段，交付了一个功能完备、"
     ...
     "时间重叠检测的完整运营规则；以单写者事务与持久化回执为核心的并发一致性与幂等设计；面向真实管理的独立控制台（场次调整、用户管理、"
     "通知发布、日志查看）与自动备份、历史归档等运营能力；覆盖单元、集成、契约、并发、故障注入、模糊与浸泡的五层自动化验证体系，"
     "以及构建、静态分析与 AddressSanitizer 三通道持续集成门禁；从 720 次争抢实验、30 次崩溃恢复实验、计时旁路实验到性能优化对比的"
     "完整实测证据链。")
h2("7.2 不足与展望")
...
# ---------- 附录 ----------
h1("附录 A  接口契约摘要")
para("系统提供 20 余个 REST 接口，统一响应封套 {code, message, data}，错误码包括 400/401/403/404/409/413/429/500/503。"
