...
para("系统通过五层自动化验证：24 个单元测试、74 项集成断言组（含 720 次并发争抢实验——实验样本内未出现超卖、30 次进程中断恢复全部正确、"
     "五组跨规则组合验证、凭据生命周期与约束感知建议评估）、49 个端点的契约测试与灰盒/文档测试、模糊稳健性与浸泡稳定性实验，以及持续集成三通道门禁（构建与测试、"
...
li("（4）构建了五层自动化验证体系：单元测试 24 个、集成断言组 74 项（T01–T73 与 CLI 检查，含 720 次并发争抢、30 次故障注入恢复、五组跨规则组合验证、凭据生命周期与约束感知建议评估）、49 个端点的契约测试与灰盒/文档测试、模糊与浸泡实验，"
...
para("api() 处理器依次执行 Host 白名单、方法检查、健康探测、Origin 校验、请求体上限与 JSON 合法性检查（含重复键拒绝）、数据库连接获取、"
     ...
     "同样返回 429；已登录用户的全部写操作经过每用户令牌桶（默认突发 30、每秒补充 1）限速。路由计数口径：分派器共 60 个路由分支"
...
h1("6 系统测试")
h2("6.1 测试策略与环境")
para("测试体系分五层：（1）Unity 单元测试 24 个，不经 HTTP 直接链接业务层与数据层；（2）集成实验 70 项断言组（编号 T01–T73 另加 CLI 数据库"
     "检查，以测试运行器 record() 调用数为准），以独立 Python 客户端"
     "驱动真实服务进程，覆盖功能、安全、并发、故障恢复、运营能力与跨规则组合语义；（3）契约测试（49 个端点逐项校验响应封套与字段类型）与灰盒/文档一致性测试；"
...
tbl(["源文件", "db.c", "http.c", "log.c", "main.c", "metrics.c", "ratelimit.c", "service.c", "util.c"],
    [["行覆盖率 %", 86.55, 89.17, 84.62, 83.53, 95.24, 86.76, 82.95, 98.08]])
h2("6.3 功能、安全与运营回归")
...
para("模糊实验以 4 线程对登录、预约、候补、改密接口持续发送畸形 JSON、超长字段、错类型、二进制垃圾与并发错乱序列共 240 次：全部请求"
     ...
     "进程工作集增长 3 MB、句柄增长 44，在浸泡窗口内未观察到工作集与句柄的持续增长趋势；该结果支持“无持续泄漏迹象”的判断，"
     "不构成对任意时长运行下无泄漏的证明。")
h2("6.9 调试过程记录")
...
para("本课题面向高校开放实验室管理场景，完整经历了需求分析、架构设计、编码实现、自动化验证与性能优化五个阶段，交付了一个功能完备、"
     "可靠性经过实测论证的 C 语言 Web 系统（v1.14.0，schema v5、17 张数据表、48 个契约端点、74 项集成断言组）。主要成果包括：容量制预约与"
...
# ---------- 附录 ----------
h1("附录 A  接口契约摘要")
para("系统共 60 个路由分支，契约测试覆盖 49 个 REST 端点的响应封套与字段类型，统一封套 {code, message, data}，错误码包括 400/401/403/404/409/413/429/500/503。"
