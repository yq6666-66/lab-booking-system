...
para("核心功能需求归纳为：场次查询（按实验室与日期，返回容量与实时占用）；预约（容量内即时生效）；候补（满员排队，按入队顺序排队）；"
     "取消与自动补位（释放与补位原子完成）；替代时段提示（满员时返回同实验室 7 天内空闲场次）；限时签到与爽约回收（超时自动释放并补位）；预约改期（同实验室原子改期，旧槽自动补位）与预约备注；预约时资源需求声明与同时段配额校验（BR12，资源随取消/爽约自动释放）；"
...
h1("6 系统测试")
h2("6.1 测试策略与环境")
para("测试体系分五层：（1）Unity 单元测试 24 个，不经 HTTP 直接链接业务层与数据层；（2）集成实验 51 项断言组，以独立 Python 客户端"
...
h2("6.3 功能、安全与运营回归")
para("集成实验共 51 项断言组（T01–T50 及两组 CLI 检查），覆盖登录授权、占用唯一、候补 FIFO 与重入队尾、重放幂等、越权与 CSRF/Origin、"
     ...
     "资源管理、利用率统计、资源声明配额、周期性发布、每周配额、签退、预约提前量与改期。最近一轮全部通过。典型新增加断言组如下：")
