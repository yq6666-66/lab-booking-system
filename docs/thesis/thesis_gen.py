...
para("系统通过五层自动化验证：24 个单元测试、75 项集成断言组（含 720 次并发争抢实验——实验样本内未出现超卖、30 次进程中断恢复全部正确、"
...
li("（4）构建了五层自动化验证体系：单元测试 24 个、集成断言组 75 项（T01–T74 与 CLI 检查，含 720 次并发争抢、30 次故障注入恢复、五组跨规则组合验证、凭据生命周期与约束感知建议评估）、49 个端点的契约测试与灰盒/文档测试、模糊与浸泡实验，"
...
h1("6 系统测试")
h2("6.1 测试策略与环境")
para("测试体系分五层：（1）Unity 单元测试 24 个，不经 HTTP 直接链接业务层与数据层；（2）集成实验 70 项断言组（编号 T01–T74 另加 CLI 数据库"
...
para("本课题面向高校开放实验室管理场景，完整经历了需求分析、架构设计、编码实现、自动化验证与性能优化五个阶段，交付了一个功能完备、"
     "可靠性经过实测论证的 C 语言 Web 系统（v1.14.0，schema v5、17 张数据表、48 个契约端点、75 项集成断言组）。主要成果包括：容量制预约与"
