...
para("模糊实验以 6 线程对登录、预约、候补、改密接口持续发送畸形 JSON、超长字段、错类型、二进制垃圾与并发错乱序列共 720 次：719 次请求"
     "得到合法封套响应（4xx 拒绝），另 1 次为建立会话的有效登录（2xx），全程零 5xx，进程存活，SQLite 完整性检查通过。浸泡实验以混合负载运行 60 秒冒烟（15,351 请求、0 错误、0 5xx），"
     "另将浸泡窗口延长至 300 秒（15,541 请求、0 错误、0 5xx，两窗口合计 30,892 请求），工作集自 8.1MB 增至 10.9MB（+2.8MB，预热期峰值 72MB 后回落）、句柄净增 38，长窗口同样未观察到持续增长趋势；上述结果支持“无持续泄漏迹象”的判断，"
     "不构成对任意时长运行下无泄漏的证明。")
h2("6.9 调试过程记录")
