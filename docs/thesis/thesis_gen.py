...
para("系统以单写者事务（BEGIN IMMEDIATE）配合容量不变量保证并发正确性；以持久化请求回执实现同编号同参数重放幂等、同编号异参数冲突拒绝；"
     ...
     "运行日志）、限时签到、场次开始提醒、自动备份轮转、爽约信用约束与时间重叠检测等运营能力，并以全局安全响应头（CSP 等）收紧浏览器攻击面。")
...
li("（3）设计并实现了面向真实运营的管理能力：独立管理员控制台（实验室与场次维护、场次开始前调整、用户停用/重置密码、全员通知发布、运行日志查看）、"
...
para("核心功能需求归纳为：场次查询（按实验室与日期，返回容量与实时占用）；预约（容量内即时生效）；候补（满员排队，按入队顺序排队）；"
     ...
     "管理员用户管理（停用立即下线、重置密码一次性展示）；记录分页与状态筛选；逐日统计图表与 CSV 导出；服务运行日志查看与级别筛选。")
h2("3.3 业务规则")
...
para("数据模型共 9 张表，如图 4-2 所示。reservations（预约）与 waitlist（候补）是核心业务表；request_receipts（请求回执）支撑幂等去重；"
     "operation_events（操作事件）提供审计；notifications（通知）承载补位、爽约、公告与提醒四类站内消息。schema 采用版本化幂等迁移："
     "v2 追加签到与取消原因列及通知表；v3 为场次表追加 capacity 列并退役单占用唯一索引；v4 为场次表追加提醒防重列 reminded_at，并为"
     "通知表的 kind 列扩展 REMIND 取值——由于 SQLite 无法修改 CHECK 约束，该迁移以\u201c建新表—拷贝—改名\u201d的方式重建通知表，"
...
h2("5.6 管理控制台与运营功能实现")
para("管理控制台为独立页面（admin.html + admin.js），以十个页签组织实验室管理、场次发布、场次管理（开始前调整时间与容量）、全员记录"
     ...
     "统计图表如图 5-3 所示。统计页的趋势图由前端以纯 DOM API 生成 SVG 双系列柱状图，不引入任何图表库。运营侧实现了自动备份线程"
...
h2("6.3 功能、安全与运营回归")
para("集成实验共 42 项断言组（T01–T40 及两组 CLI 检查），覆盖登录授权、占用唯一、候补 FIFO 与重入队尾、重放幂等、越权与 CSRF/Origin、"
     "畸形与超长输入、过期场次、外部写锁退避、禁用候补跳过、替代时段对账、统计对账、会话清理、签到窗口、分页参数、CSV 导出、改密会话失效、"
     "混合负载连接复用，以及运营扩展的角色登录、场次调整、通知发布、用户管理、场次提醒、自动备份、爽约信用、时间重叠、历史归档与日志查看。"
     "最近一轮全部通过。典型新增加断言组如下：")
