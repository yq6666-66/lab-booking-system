...
para("核心功能需求归纳为：场次查询（按实验室与日期，返回容量与实时占用）；预约（容量内即时生效）；候补（满员排队，按入队顺序排队）；"
     ...
     "管理员用户管理（停用立即下线、重置密码一次性展示）；记录分页与状态筛选；逐日统计图表与 CSV 导出；服务运行日志查看与级别筛选；实验室资源清单维护（名称/规格/数量/可用状态）与按实验室的资源利用率与实机时统计导出、周期性场次发布、每周预约配额（BR13）、iCalendar 日历导出。")
h2("3.3 业务规则")
...
h1("6 系统测试")
h2("6.1 测试策略与环境")
para("测试体系分五层：（1）Unity 单元测试 24 个，不经 HTTP 直接链接业务层与数据层；（2）集成实验 48 项断言组，以独立 Python 客户端"
...
h2("6.3 功能、安全与运营回归")
para("集成实验共 48 项断言组（T01–T47 及两组 CLI 检查），覆盖登录授权、占用唯一、候补 FIFO 与重入队尾、重放幂等、越权与 CSRF/Origin、"
     ...
     "资源管理、利用率统计、资源声明配额、周期性发布、每周配额与签退。最近一轮全部通过。典型新增加断言组如下：")
