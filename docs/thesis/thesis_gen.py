...
h1("6 系统测试")
h2("6.1 测试策略与环境")
para("测试体系分五层：（1）Unity 单元测试 24 个，不经 HTTP 直接链接业务层与数据层；（2）集成实验 50 项断言组，以独立 Python 客户端"
...
h2("6.3 功能、安全与运营回归")
para("集成实验共 50 项断言组（T01–T49 及两组 CLI 检查），覆盖登录授权、占用唯一、候补 FIFO 与重入队尾、重放幂等、越权与 CSRF/Origin、"
     ...
     "资源管理、利用率统计、资源声明配额、周期性发布、每周配额、签退与预约提前量。最近一轮全部通过。典型新增加断言组如下：")
