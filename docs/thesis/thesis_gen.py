...
li("（2）可执行候补策略：突破传统 FIFO 的队首阻塞，递补事务按（优先级降序，编号升序）扫描并暂跳暂时不可执行的候选、保留其序号，配合限时保留（HELD）与优先级抢占/信用补偿闭环；对照实验显示名额利用率与候补成功率由 0 提升至 1.0；")
...
li("BR19 可执行 FIFO：候补递补按（优先级降序,编号升序）扫描队列，停用者跳过、临时时间冲突者暂跳并保留原序号，只递补第一个当前可执行的候选；strict 策略可回退为队首阻塞语义；")
...
para("候补策略：默认可执行 FIFO——递补事务按（优先级降序，编号升序）扫描队列，账号停用者置 SKIPPED，临时时间冲突者暂跳并保留原序号，"
     "只递补第一个当前可执行的候选，避免队首暂时不可执行时阻塞整条队列；--waitlist-strategy=strict 可回退为队首阻塞语义，供对照实验。"
