...
static int app_main(int argc,char **argv){
 Config c={"data/lab.db","web",8080,NULL,NULL};int seed=0,init=0,check=0;
...
#endif
  else {fprintf(stderr,"Usage: lab-booking --db FILE --web DIR --port PORT [--seed --init-only] [--check]\n");return 2;}
 }
 ...
 if(db_num(&db,"PRAGMA user_version","")>1){fprintf(stderr,"Unsupported schema version\n");db_close(&db);return 1;}
