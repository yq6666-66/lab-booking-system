...
static int gen_demo(DB *db,int days,const char *password){
 ...
 Config c={"data/lab.db","web",8080,900,30,30,1,5,900,500,NULL,NULL,NULL,NULL,1800,21600,0,0,0,0,0,0,0,NULL};int seed=0,init=0,check=0,demo_days=0;
 for(int i=1;i<argc;i++){
  ...
  else if(!strcmp(argv[i],"--waitlist-strategy")&&i+1<argc){const char *v=argv[++i];if(strcmp(v,"strict")&&strcmp(v,"executable")&&strcmp(v,"weighted")){fprintf(stderr,"Waitlist strategy must be strict, executable or weighted\n");return 2;}c.waitlist_strict=!strcmp(v,"strict");c.waitlist_strategy=!strcmp(v,"strict")?1:!strcmp(v,"weighted")?2:0;}
 else if(!strcmp(argv[i],"--waitlist-daily-limit")&&i+1<argc){Id n=0;if(!parse_id(argv[++i],&n)||n>100){fprintf(stderr,"Waitlist daily limit must be 0..100 (0 disables)\n");return 2;}c.waitlist_daily_limit=(int)n;}
  ...
  /* r24 P2-6 出站通知：file 渠道（JSONL 追加），默认关闭零行为变化 */
  else if(!strcmp(argv[i],"--notify-file")&&i+1<argc)c.notify_file=argv[++i];
#ifdef TEST_FAULTS
  else if(!strcmp(argv[i],"--fault")&&i+1<argc)c.fault=argv[++i];
  else if(!strcmp(argv[i],"--fault-request")&&i+1<argc)c.fault_request=argv[++i];
#endif
  else {fprintf(stderr,"Usage: lab-booking --db FILE --web DIR --port PORT [--checkin-window SEC] [--sweep-interval SEC] [--rate-burst N] [--rate-refill-sec SEC] [--login-max-fails N] [--login-lockout SEC] [--slow-ms MS] [--backup DEST] [--remind-sec SEC] [--backup-interval SEC] [--demo-days N] [--notify-file FILE] [--seed --init-only] [--check]\n");return 2;}
 }
 ...
 clock_configure(&c);notify_configure(&c);
 _mkdir("data");DB db={0};
